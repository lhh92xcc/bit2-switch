...
const LEGACY_CC_SWITCH_CODEX_MODEL_PROVIDER_ID: &str = "bit2switch";
...
/// 迁移前 jsonl / state DB 均备份到 `~/.bit2-switch/backups/codex-official-history-unify-v1/`。
...
"#
            ...
            Provider::with_id(
                "legacy-bit2switch".to_string(),
                "Legacy bit2-switch".to_string(),
                serde_json::json!({
                    "auth": {},
                    "config": r#"model_provider = "bit2switch"

[model_providers.bit2switch]
name = "AIHubMix"
base_url = "https://aihubmix.example/v1"
"#
                }),
...
"#
        ...
        assert_eq!(
            source_provider_ids,
            source_ids(&["aihubmix", "bit2switch", "rightcode"])
        );
        ...
        fs::write(
            ...
            concat!(
                ...
                "{\"type\":\"session_meta\",\"payload\":{\"id\":\"s3\",\"model_provider\":\"bit2switch\"}}\n",
        ...
        assert_eq!(
            ...
            source_ids(&["legacy-bit2switch", "rightcode"])
        );
        ...
        let bit2switch_config: toml::Value =
            toml::from_str(&config_provider_id("legacy-bit2switch")).expect("parse bit2switch config");
        assert_eq!(
            bit2switch_config
                .get("model_provider")
                .and_then(|value| value.as_str()),
        ...
        assert!(bit2switch_config
            .get("model_providers")
            .and_then(|value| value.get("bit2switch"))
            .is_none());
...
"#
    ...
    #[test]
    fn collects_legacy_bit2switch_provider_id_from_stored_config() {
        let db = Database::memory().expect("memory db");
        let mut provider = Provider::with_id(
            "generated-uuid".to_string(),
            ...
            serde_json::json!({
                "auth": {},
                "config": "model_provider = \"bit2switch\"\n\n[model_providers.bit2switch]\nname = \"AIHubMix\"\nbase_url = \"https://aihubmix.example/v1\""
            }),
            None,
        );
        provider.category = Some("aggregator".to_string());
        ...
        let ids = collect_source_model_provider_ids(&db).expect("collect ids");
        assert!(ids.contains("bit2switch"));
        assert!(ids.contains("aihubmix"));
        assert!(!ids.contains("generated-uuid"));
    }
...
"#
    ...
    #[test]
    fn migrates_legacy_bit2switch_provider_template_to_custom() {
        let db = Database::memory().expect("memory db");
        let provider = Provider::with_id(
            "legacy-bit2switch".to_string(),
            "Legacy bit2-switch".to_string(),
...
"#
        ...
        assert_eq!(
            ...
            vec!["legacy-bit2switch".to_string()]
        );

        let saved = db
            .get_provider_by_id("legacy-bit2switch", "codex")
            .expect("get provider")
            .expect("provider exists");
        let config_text = saved
            .settings_config
