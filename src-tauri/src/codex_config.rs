...
/// provider is routed through bit2-switch.  A dedicated id is an ownership
...
pub const CC_SWITCH_CODEX_OFFICIAL_PROXY_PROVIDER_ID: &str = "bit2-switch-official";
pub const CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "bit2-switch-model-catalog.json";
...
/// this per the vendors' official Codex docs. Also doubles as bit2-switch's
...
struct CodexManagedOAuthLiveAuthMarker {
    version: u32,
    /// bit2-switch 本地托管账号 ID，用于区分同一 ChatGPT workspace 下的登录。
    account_id: String,
...
/// Rollback point for the bit2-switch-owned model catalog. Catalog projection
...
///
/// - `ProxyChat`: bit2-switch's proxy takes over and converts Responses<->Chat,
...
pub enum CodexCatalogToolProfile {
    ProxyChat,
    NativeResponses,
    /// Codex talks (through bit2-switch's proxy) to a native Anthropic Messages
...
/// 作为稳定的所有权谓词；bit2-switch 的本地账号 ID 单独记录在 marker 中。
...
/// Build the native-shaped ChatGPT auth bundle shared by bit2-switch and Codex CLI.
...
///
/// 删除谓词同时校验 bit2-switch marker 中的本地账号 ID 与原生 auth.json 中的
...
/// 判断给定的 Codex `auth` 是否属于指定的 bit2-switch 本地托管账号。
///
/// 原生 `tokens.account_id` 是 workspace ID，可能被多个本地账号共享；因此必须同时
/// 命中 bit2-switch marker 中的本地账号 ID，不能只按 auth.json 内容判断。
///
...
/// network request. Codex CLI does not share bit2-switch's process lock, so this
...
/// catalog (models.json) that bit2-switch mirrors verbatim. Matched against
...
/// runs through bit2-switch's converter (gpt-5.5 template contract) and the
...
fn set_codex_model_catalog_json_field(
    ...
    match catalog_path {
        Some(_) => {
            // Only claim the pointer when it is absent or already bit2-switch-owned.
...
/// but only when it carries bit2-switch's own `"disabled"` sentinel, so switching
...
/// lifecycle is bound to the bit2-switch catalog pointer so the field is set/cleaned
...
/// bit2-switch–maintained catalog file referenced by `~/.codex/config.toml` and
...
/// bit2-switch–generated file (identified by filename
/// `bit2-switch-model-catalog.json`). A user-managed external catalog file is
...
/// Read the bit2-switch Codex model catalog file with a size cap.
...
/// Given `config.toml` text, resolve the on-disk path of the bit2-switch–owned
...
pub(crate) fn resolve_cc_switch_catalog_path(
    ...
    // /etc）能让 `link/bit2-switch-model-catalog.json` 通过上面的检查，读取却
...
/// bit2-switch's own record (extract/backfill), never read by Codex.
...
/// bit2-switch-owned provider id used by the legacy-shape normalization below.
/// Not a Codex reserved id, so an injected token lands inside the table.
const CODEX_MIGRATED_PROVIDER_ID: &str = "bit2-switch";

/// Pick the first free bit2-switch-owned provider id (`bit2-switch`,
/// `bit2-switch-2`, …) so migrations never overwrite a user-authored table.
...
/// means "switch reports success, Codex refuses to start" — older bit2-switch
...
/// table is renamed losslessly to the first free bit2-switch id (nothing
...
/// ("provider name must not be empty"). Historic bit2-switch updates and
...
/// a custom provider table named `bit2-switch`. Before Codex 0.149 this shape
...
pub fn apply_codex_official_proxy_route(
    ...
    // Clean only bit2-switch's placeholder from every stale provider table. Real
...
/// Whether a live Codex config is the official route projected by bit2-switch.
...
/// Remove only the official takeover route owned by bit2-switch. This is a
...
fn plan_codex_live_write(
    ...
    if category == Some("official") {
        // Official configs seeded by older bit2-switch versions can carry
        ...
        // idle leftovers from older bit2-switch versions.
    ...
    // into a bit2-switch-owned custom table before the safety gates run.
...
mod tests {
    ...
    fn seed_rotated_managed_codex_live_state() -> CodexLiveTestState {
        ...
        crate::config::write_text_file(
            ...
            "# cas-guard-sentinel\nmodel = \"gpt-5.5\"\nmodel_catalog_json = \"bit2-switch-model-catalog.json\"\n",
        )
...
"#;
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        let with_catalog = "model_catalog_json = \"bit2-switch-model-catalog.json\"\n";
...
"#;
    ...
    fn third_party_route_without_token_slot_detection() {
        ...
        // stays a bit2-switch-only record).
...
"#;
        ...
        assert!(
            normalized.contains("model_provider = \"bit2-switch\""),
            "routing must move to the bit2-switch table; got:\n{normalized}"
        );
        assert!(
            normalized.contains("[model_providers.bit2-switch]"),
        ...
        assert!(
            ...
            "token must land inside the bit2-switch table; got:\n{injected}"
        );
        assert_eq!(
    ...
    fn legacy_reroute_normalization_never_overwrites_a_user_cc_switch_table() {
        // A user-authored [model_providers.bit2-switch] proves nothing about
...
"#;
        ...
        assert!(
            normalized.contains("model_provider = \"bit2-switch-2\"")
                && normalized.contains("[model_providers.bit2-switch-2]"),
    ...
    fn stale_reserved_tables_are_renamed_with_fallback_aware_routing() {
        // Older bit2-switch takeover projections created reserved
...
"#;
        ...
        assert!(
            keyless.contains("model_provider = \"bit2-switch\"") && keyless.contains("own-key"),
...
"#;
        let local = prepare_codex_provider_live_config(&json!({}), unauthenticated_stale)
            .expect("prepare live config without token");
        assert!(
            local.contains("model_provider = \"bit2-switch\"")
...
"#;
        let scoped = prepare_codex_provider_live_config(&json!({}), scoped_token_stale)
            .expect("prepare live config without token");
        assert!(
            scoped.contains("model_provider = \"bit2-switch\"")
...
"#;
        ...
        assert!(
            snapped.contains("model_provider = \"openai\"")
                ...
                && snapped.contains("[model_providers.bit2-switch]"),
        ...
        assert!(
            official.contains("model_provider = \"openai\"")
                && official.contains("[model_providers.bit2-switch]"),
...
"#;
        ...
        assert!(
            !cleaned.contains("[model_providers.ollama]")
                ...
                && cleaned.contains("[model_providers.bit2-switch]")
                && cleaned.contains("[model_providers.bit2-switch-2]")
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/bit2-switch-model-catalog.json");
...
"#;
    ...
    fn native_web_search_field_removes_own_sentinel_when_not_disabled() {
        ...
        // removing bit2-switch's own "disabled" sentinel.
...
"#;
        ...
        assert!(
            ...
            "bit2-switch's disabled sentinel should be removed when not native"
        );
    }
    ...
    fn native_web_search_field_preserves_user_value() {
        ...
        // only bit2-switch's "disabled" sentinel is owned/removable.
...
"#;
    ...
    fn resolve_catalog_path_accepts_cc_switch_owned_file() {
        let base = PathBuf::from("/tmp/.codex");
        let config = r#"model_catalog_json = "/tmp/.codex/bit2-switch-model-catalog.json"
"#;
...
model = "glm-5"
"#;
        // Simulate a WSL UNC path as bit2-switch would see it on Windows;
        // the function now writes just the relative filename.
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\bit2-switch-model-catalog.json");
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/bit2-switch-model-catalog.json");
    ...
    fn set_catalog_json_none_removes_cc_switch_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/bit2-switch-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove bit2-switch-owned field regardless of path format"
        );
    }
...
"#;
    ...
    fn set_catalog_json_some_preserves_user_owned_catalog() {
        // When bit2-switch generates a catalog (Some arm), it must still respect a
        // user-managed external catalog file instead of clobbering it with the
        // bit2-switch-owned filename. Only an absent or bit2-switch-owned pointer is
...
"#;
    ...
    fn resolve_catalog_rejects_absolute_path_outside_config_dir() {
        let config_text = r#"model_catalog_json = "/tmp/secret/bit2-switch-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_accepts_absolute_path_inside_config_dir() {
        let config_text = r#"model_catalog_json = "/home/user/.codex/bit2-switch-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_rejects_traversal_to_parent_directory() {
        let config_text = r#"model_catalog_json = "../bit2-switch-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_rejects_symlink_escaping_config_dir() {
        ...
        // "link/bit2-switch-model-catalog.json" 词法上在 base 内，真实读取却落到
        ...
        let config_text = r#"model_catalog_json = "link/bit2-switch-model-catalog.json"
"#;
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
            result, None,
