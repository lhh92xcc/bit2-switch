...
///
/// Input (from bit2-switch UI / database):
/// ```json
...
/// `providers:` dict. bit2-switch treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by bit2-switch)
...
/// keeping bit2-switch free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay bit2-switch must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a bit2-switch edit to an unrelated field would silently
...
// bit2-switch fills that gap by reading/writing the whole file as a markdown
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later bit2-switch edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a bit2-switch toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
