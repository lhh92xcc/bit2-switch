...
pub(crate) const TOOL_RESULT_MEDIA_MOVED_MARKER: &str =
    "[bit2-switch: tool result media moved to the following user message]";
pub(crate) const TOOL_RESULT_MEDIA_ATTACHED_MARKER: &str =
    "[bit2-switch: tool result media attached as native media]";
...
pub(crate) fn queue_chat_tool_output_media(
    ...
    pending_media.push(json!({
        "type": "text",
        "text": format!("[bit2-switch: media output of tool call {call_id}]")
    }));
    ...
    match value {
        Value::String(text) => {
            ...
            if should_omit {
                ...
                *text = format!("[bit2-switch: omitted {byte_len} bytes]");
            }
        }
        Value::Array(items) => {
            for item in items {
...
mod tests {
    ...
    fn chat_plan_clamps_residual_base64_inside_json_string_before_serializing() {
        ...
        assert!(plan
            .tool_content
            .contains("[bit2-switch: omitted 20000 bytes]"));
    ...
    fn clamp_preserves_long_text_but_removes_data_and_base64_payloads() {
        ...
        assert!(value["data_url"]
            .as_str()
            .unwrap()
            .starts_with("[bit2-switch: omitted "));
        assert!(value["raw"]
            .as_str()
            .unwrap()
            .starts_with("[bit2-switch: omitted "));
