...
pub enum SkillStorageLocation {
    /// bit2-switch 管理目录 (~/.bit2-switch/skills/)
    #[default]
    CcSwitch,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.bit2-switch/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.bit2-switch/skill-backups/）
        ...
        let (backup_path, preserved_pi_path, pi_cleanup_incomplete) =
            match Self::require_valid_directory(&skill.directory) {
                Ok(directory) => {
                    ...
                    // 能验证为 bit2-switch 部署的副本；其余路径保留并返回警告。
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 bit2-switch 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "bit2-switch".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 bit2-switch 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "bit2-switch".to_string()));
    ...
    fn resolve_uninstall_backup_source_excluding(
        ...
        // 返回值会被整目录复制进 ~/.bit2-switch/skill-backups/ 并由 get_skill_backups
...
mod tests {
    ...
    fn validate_repo_ref_accepts_real_world_coordinates() {
        ...
        for branch in [
            ...
            assert!(
                SkillService::validate_repo_ref("farion1231", "bit2-switch", branch).is_ok(),
    ...
    fn validate_repo_ref_accepts_the_empty_branch_sentinel() {
        ...
        assert!(
            SkillService::validate_repo_ref("farion1231", "bit2-switch", "").is_ok(),
    ...
    fn restore_from_backup_rejects_traversal_directory_in_metadata() {
        ...
        // SSOT 位于 {home}/.bit2-switch/skills，"../../pwned-restore" 若生效会写到 {home}/pwned-restore。
    ...
    fn uninstall_rejects_traversal_directory_from_db_row() {
        ...
        // 与 import_sql_string_for_sync 的效果一致）。SSOT = {home}/.bit2-switch/skills，
    ...
    fn migrate_storage_retargets_an_equivalent_relative_pi_symlink() {
        ...
        std::os::unix::fs::symlink(Path::new("../../.bit2-switch/skills/test-skill"), &pi_skill)
