...
fn restore_live_settings_for_provider_backfill(
    ...
    // `modelCatalog` is a bit2-switch–private field whose SSOT is the DB. Live's
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        AppType::Codex => {
            ...
            // `modelCatalog` is a bit2-switch private field that lives only in
            // the DB SSOT plus the `bit2-switch-model-catalog.json` projection
...
/// into the bit2-switch database. Each provider found will be added to the
