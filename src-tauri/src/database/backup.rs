...
const CC_SWITCH_SQL_EXPORT_HEADER: &str = "-- bit2-switch SQLite 导出";
...
impl Database {
    ...
    fn validate_cc_switch_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        Err(AppError::localized(
            ...
            "仅支持导入由 bit2-switch 导出的 SQL 备份文件。",
            "Only SQL backups exported by bit2-switch are supported.",
        ))
    }
    ...
    {
        let db_path = get_app_config_dir().join("bit2-switch.db");
        if !db_path.exists() {
            return Ok(None);
        }
    ...
    /// Validate that the external SQL created a recognizable bit2-switch schema.
    ///
    ...
    fn validate_imported_schema(conn: &Connection) -> Result<(), AppError> {
        ...
        if !missing.is_empty() {
            ...
            return Err(AppError::localized(
                ...
                format!("导入的 SQL 缺少 bit2-switch 必需表：{names}"),
                format!("The imported SQL is missing required bit2-switch tables: {names}"),
            ));
        }
        Ok(())
    }
...
mod tests {
    ...
    impl TestHomeGuard {
        fn new() -> Self {
            ...
            let config_dir = temp_dir.path().join(".bit2-switch");
            std::fs::create_dir_all(&config_dir).expect("create isolated config directory");
            std::fs::File::create(config_dir.join("bit2-switch.db"))
    ...
    fn import_rejects_cross_file_statements_and_leaves_no_file_behind() -> Result<(), AppError> {
        ...
        for (label, template) in cases {
            let target = test_home
                .path()
                .join(format!("bit2-switch-authorizer-{label}.sqlite"));
    ...
    fn import_rejects_header_only_sql_and_keeps_existing_database() -> Result<(), AppError> {
        ...
        assert!(
            error.to_string().contains("required bit2-switch tables")
                || error.to_string().contains("bit2-switch 必需表"),
    ...
    fn import_rejects_truncated_open_transaction_and_keeps_live_database() -> Result<(), AppError> {
        ...
        let truncated = exported
            .strip_suffix("COMMIT;\nPRAGMA foreign_keys=ON;\n")
            .expect("bit2-switch export should end with a committed transaction");
        ...
        let temporary_files = std::fs::read_dir(&backup_dir)
            ...
            .filter(|entry| {
                entry
                    .file_name()
                    .to_string_lossy()
                    .starts_with(".bit2-switch-backup-")
            })
            .count();
        assert_eq!(
            temporary_files, 0,
