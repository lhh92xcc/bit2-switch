...
//! bit2-switch only manages explicit provider entries in `models.json`.
...
pub(crate) fn replace_pi_provider(
    ...
    if current != expected {
        return Err(AppError::Conflict(format!(
            "Pi provider '{provider_key}' changed outside bit2-switch"
        )));
    }
...
/// Validate the shape bit2-switch can persist as one
...
fn ensure_models_revision(path: &Path, expected_revision: &str) -> Result<(), AppError> {
    ...
    if actual_revision == expected_revision {
        ...
        Err(AppError::Conflict(format!(
            "Pi models.json changed outside bit2-switch: {}",
            path.display()
        )))
    }
}
...
mod tests {
    ...
    fn provider_node_accepts_unknown_native_fields() {
        ...
        validate_provider_node("bit2-switch-example", &value).expect("valid provider");
    }
    ...
    fn provider_node_ownership_depends_on_models_json_membership() {
        ...
        validate_provider_node("bit2-switch-example", &oauth)
    ...
    fn newly_created_models_file_and_agent_directory_are_private() {
        ...
        let _agent = test_support::TestAgentDir::new();
        insert_pi_provider("bit2-switch-private", &provider()).expect("write private models file");
    ...
    fn stale_models_revision_does_not_overwrite_an_external_edit() {
        ...
        let replacement = json!({"providers": {"bit2-switch": provider()}});
