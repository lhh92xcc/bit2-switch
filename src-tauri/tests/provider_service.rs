...
fn provider_service_switch_codex_preserved_login_normalizes_legacy_reroute_config() {
    ...
    // normalize the config into a bit2-switch-owned custom table with the key
...
"#;
    ...
    assert!(
        live_config.contains("[model_providers.bit2-switch]")
            && live_config.contains("base_url = \"https://relay.example/v1\"")
            && live_config.contains("experimental_bearer_token = \"third-party-key\""),
        "routing and key must move into the bit2-switch provider table; got:\n{live_config}"
    );
...
"#;
    ...
    assert!(
        live_config.contains("[model_providers.bit2-switch]"),
        "a bit2-switch provider table must be created; got:\n{live_config}"
    );
    assert_eq!(
...
fn switch_packycode_gemini_updates_security_selected_type() {
    ...
    // Gemini security settings are written to ~/.gemini/settings.json, not ~/.bit2-switch/settings.json
...
/// 下一个供应商；供应商专属字段、密钥与 bit2-switch 注入产物绝不进片段；
...
fn switch_codex_syncs_shared_keys_from_live_into_common_config() {
    ...
    // disable_response_storage + bit2-switch 注入产物 + MCP 同步投影
